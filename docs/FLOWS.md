# User-Facing Flows

Status legend: ✅ done | ⚠️ partial/known issue | 🚧 in progress | ❌ not started

| Flow | Status | Notes |
|---|---|---|
| Task 1 — CSV merge into SQLite | ✅ | Spec: docs/specs/001-task1-merge-pipeline.md — run `ingest/ingest.py` |
| Task 2 — n8n skill auto-tagging | ✅ | Spec: docs/specs/002-task2-n8n-automation.md — n8n/skill-tagging-flow.json |
| Task 3 — Audio submission (record/upload) | ✅ | Spec: docs/specs/003-task3-audio-app.md — run `streamlit run audio_app/app.py` |
| Task 3 — Audio submissions listing/playback | ✅ | Spec: docs/specs/003-task3-audio-app.md |
| Task 4 — Data issues report | ❌ | Spec: docs/specs/004-task4-data-issues-report.md |
| Task 5 — Scaling stretch doc | ❌ | Spec: docs/specs/005-task5-scaling-doc.md |
